...
public class QuadcopterController : MonoBehaviour
{
	...
	public void AddControlls(KeyCode rosCommand = KeyCode.None)
	{
		//yawDestination = 0;
		sideAmount = 0;
		forwardAmount = 0;
		...
		if (Input.GetKey(KeyCode.Q) || rosCommand == KeyCode.Q)
		{
			throttle += Time.deltaTime * throttleSpeed;
		}

		if (Input.GetKey(KeyCode.E) || rosCommand == KeyCode.E)
		{
			throttle -= Time.deltaTime * throttleSpeed;
		}

		if (Input.GetKey(KeyCode.W) || rosCommand == KeyCode.W)
		{
			forwardAmount += Time.deltaTime * forwardSpeed;
		}

		if (Input.GetKey(KeyCode.S) || rosCommand == KeyCode.S)
		{
			forwardAmount -= Time.deltaTime * forwardSpeed;
		}

		if (Input.GetKey(KeyCode.A) || rosCommand == KeyCode.A)
		{
		...
		if (Input.GetKey(KeyCode.D) || rosCommand == KeyCode.D)
		{
